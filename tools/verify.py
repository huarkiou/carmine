...
def verify_sales(filepath):
    ...
    for name in wb.sheetnames:
        ...
        for row in range(2, ws.max_row + 1):
            ...
            for suffix in ["汽车", "集团"]:
                if manu.endswith(suffix) and manu not in [
                    "上汽通用五菱",
                    "东风日产",
                    "鸿蒙智行",
                ]:
    ...
    for name in wb.sheetnames:
        ...
        for row in range(2, ws.max_row + 1):
            ...
            if all_empty:
                empty_rows += 1
        print(
            f"  Sheet '{name}': {ws.max_row - 1} params x {ws.max_column - 1} specs, {empty_rows} all-empty rows"
        )
...
if __name__ == "__main__":
    path = (
        sys.argv[1] if len(sys.argv) > 1 else "output/latest/汽车销量排行-近6个月.xlsx"
    )
